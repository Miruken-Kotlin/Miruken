...
class ValidationErrorTest {
    @Test fun `Adds simple error`() {
        ...
        assertEquals("[name] name can't be empty\n", outcome.error)
    ...
    @Test fun `Adds nested error`() {
        ...
        assertEquals("[name] name can't be empty\n", outcome["company"])
        assertEquals("[company] [name] name can't be empty\n", outcome.error)
    ...
    @Test fun `Adds indexed error`() {
        ...
        assertEquals("[addresses] [0] city can't be empty\n", outcome["company"])
        assertEquals("[company] [addresses] [0] city can't be empty\n", outcome.error)
        assertTrue(outcome.culprits.contains("company"))
        val company = outcome.getOutcome("company")
        assertFalse(company!!.isValid)
        assertEquals("[addresses] [0] city can't be empty\n", company.error)
        assertEquals("[0] city can't be empty\n", company["addresses"])
    ...
    @Test fun `Collects validation errors`() {
        ...
        assertEquals("[email] email is not vaild; email must be at least 5 characters\n", outcome.error)
